...
public class BootstrapProgressBarGroup extends BootstrapGroup implements ProgressView, RoundableView {
    ...
    private final ProgressBarX emptyProgressBar = new ProgressBarX(getContext());
    ...
    private ProgressBarX retrieveChild(int i) {
        View view = getChildAt(i);

        if ((view instanceof ProgressBarX)) {
            return (ProgressBarX) view;
        }
        else {
     ...
     * @param progressBarX the child View
     */
    public void onProgressChanged(ProgressBarX progressBarX){
        updateBootstrapGroup();
    }
