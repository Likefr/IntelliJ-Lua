...
import com.qklua.editor.LuaLexer;
import com.qklua.editor.LuaTokenTypes;
...
public class Lexer {
	...
	//自定义关键词
	public final static int NAMEA =5;
	public final static int NAMEB =6;
	public final static int NAMEC = 7;
	public final static int NAMED = 8;
	public final static int NAMEE = 9;
	public final static int NAMEF = 11;
	public final static int NAMEG = 12;
	public final static int NAMEH = 13;
	public final static int NAMEI = 14;
	public final static int NAMEJ = 15;
	public final static int NAMEK = 16;
	public final static int NAMEL = 17;
    public final static int NAMEM = 18;
	public final static int NAMEN = 19;
	public final static int NAMEO = 22;
	public final static int NAMEP = 23;
	public final static int NAMEQ = 24;
	public final static int NAMER = 25;
	public final static int NAMES = 26;
	public final static int NAMET = 27;
                    ...
                    switch (type) {
                        ...
                        case NAME:
                            ...
                            if (lastType == LuaTokenTypes.FUNCTION) {
								...
								//自定义 k,v类关键词
                            }else if (language.isBasePackagea(name)) {
                                tokens.add(new Pair(len, NAME));
							}else if (lastType == LuaTokenTypes.DOT && language.isBasePackage(lastName) && language.isBaseWord(lastName, name)) {
                                //标准库函数
                                tokens.add(new Pair(len, NAME));
                            } else if (lastType == LuaTokenTypes.DOT && language.isBasePackagea(lastName) && language.isBaseWorda(lastName, name)) {
                                //标准库函数
                                tokens.add(new Pair(len, NAME));
							...
							//自定义namea-namel 高亮
							else if (language.isNamea(name)) {
                                tokens.add(new Pair(len, NAMEA));
                            } 
							else if (language.isNameb(name)) {
                                tokens.add(new Pair(len, NAMEB));
                            } 
							else if (language.isNamec(name)) {
                                tokens.add(new Pair(len, NAMEC));
                            } 
							else if (language.isNamed(name)) {
                                tokens.add(new Pair(len, NAMED));
                            } 

							else if (language.isNamee(name)) {
                                tokens.add(new Pair(len, NAMEE));
                            } 
							else if (language.isNamef(name)) {
                                tokens.add(new Pair(len, NAMEF));
                            } 
							else if (language.isNameg(name)) {
                                tokens.add(new Pair(len, NAMEG));
                            } 
							else if (language.isNameh(name)) {
                                tokens.add(new Pair(len, NAMEH));
                            } 
							else if (language.isNamei(name)) {
                                tokens.add(new Pair(len, NAMEI));
                            } 
							else if (language.isNamej(name)) {
                                tokens.add(new Pair(len, NAMEJ));
                            } 
							else if (language.isNamek(name)) {
                                tokens.add(new Pair(len, NAMEK));
                            } 

							else if (language.isNamel(name)) {
                                tokens.add(new Pair(len, NAMEL));
                            } 
							else if (language.isNamem(name)) {
                                tokens.add(new Pair(len, NAMEM));
                            } 
							else if (language.isNamen(name)) {
                                tokens.add(new Pair(len, NAMEN));
                            } 
							else if (language.isNameo(name)) {
                                tokens.add(new Pair(len, NAMEO));
                            } 
							else if (language.isNamep(name)) {
                                tokens.add(new Pair(len, NAMEP));
                            } 
							else if (language.isNameq(name)) {
                                tokens.add(new Pair(len, NAMEQ));
                            } 
							else if (language.isNamer(name)) {
                                tokens.add(new Pair(len, NAMER));
                            } 
							else if (language.isNames(name)) {
                                tokens.add(new Pair(len, NAMES));
                            } 
							else if (language.isNamet(name)) {
                                tokens.add(new Pair(len, NAMET));
                            } 
                    ...
                    case SINGLE_SYMBOL_WORD:
                        ...
                        if (stateChanged) {
                            ...
                            if (currentCharInWord > 0) {
								...
								//自定义添加namea-namel关键词判断

								else if (language.isNamea(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEA;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNameb(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEB;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamec(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEC;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamed(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMED;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamee(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEE;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamef(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEF;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNameg(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEG;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNameh(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEH;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamei(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEI;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamej(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEJ;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamek(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEK;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamel(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEL;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamem(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEM;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamen(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEN;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNameo(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEO;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamep(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEP;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNameq(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMEQ;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamer(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMER;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNames(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMES;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
								else if (language.isNamet(new String(candidateWord, 0, currentCharInWord))) {
                                    spanStartPosition = workingPosition - currentCharInWord;
                                    state = NAMET;
                                    tokens.add(new Pair(spanStartPosition, state));
                                } 
